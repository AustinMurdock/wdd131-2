...
function buildHtml(recipe) {
    let recipeTagsHtml = ``;
    let recipeRatingHtml = ``;
    recipe.tags.forEach(e => {
        recipeTagsHtml += `\n<li>${e}</li>`;
    });
    for (let i=1; i<6; i++) {
        if (i <= recipe.rating) {
...
searchSubmit.addEventListener("click", e => {
    ...
    gallery.innerHTML = ``;
    let runningHtml = ``;
    let filtRecipes = recipes.filter(r => {
        return (
    ...
    let sortRecipes = filtRecipes.sort((a,b) => {
        if (a.name < b.name) {
            return -1;
        }
    ...
    sortRecipes.forEach(r => {
        runningHtml += buildHtml(r)
    });
    gallery.innerHTML = runningHtml;
});

let randomRecipeI = Math.floor(Math.random() * recipes.length);
gallery.innerHTML += buildHtml(recipes[randomRecipeI]);
